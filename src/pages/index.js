  ...
  const bind = useGesture({
    onDrag: ({
      ...
      if (down && distance > window.innerHeight / 4)
        cancel(
          setIndex(clamp(index + (yDir > 0 ? -1 : 1), 0, brands.length - 1))
        );
      set((i) => {
    ...
    onWheel: ({ wheeling, distance }) => {
      set((i) => {
        const sc = wheeling ? 1 - distance / window.innerHeight / 100 : 1;
        ...
        >
          <AnimatedBrandSection
            animation={{
              ...
              background: brands[i].background,
            }}
            advance={setIndex}
            current={index}
            {...brands[i]}
